...
import { ExtractedClaimsSchema } from "./schemas";
 ...
 * 嘘として保存されない取りこぼしが出る。
 *
 * 呼び出し口は `extractClaims` 1つに固定してある。将来ローカルの軽量モデルに
 * 差し替えるとき、pipeline 側は触らずこの関数の中身だけ入れ替えられる。
 */
...
${text}`;
  ...
  let parsed;
  try {
    parsed = ExtractedClaimsSchema.parse(JSON.parse(raw));
  } catch (err) {
    throw new Error(`Failed to parse claims output: ${err}`);
  }
  return groundClaims(parsed.claims, canonFacts, normalize);
}
