 ...
 *
 * バックエンドは新しい嘘の追加枠と材料を渡す。ユーザーへの応じ方と、
 * 枠を使うかは LLM が選ぶ。会話のケース別手順をプロンプトに積み増さない。
 */
...
まずユーザーの質問や感想に応じてください。新しい嘘は「今回の追加枠」の範囲で、返答になじむときだけ混ぜてよく、追加は義務ではありません。
追加枠が0でも、既に語ったことや本物の設定、感想で会話を続けられます。
既に語った設定（このセッションで導入済みの嘘）は本作の事実として扱い、絶対に矛盾させないでください。

## としお
会話にはもう1人「としお」がいて、ときどき割り込んで大げさな考察を語ります。履歴で「【としお】」で始まる発話は、あなたではなくとしおの発言です。
としおの考察は否定も断定的な肯定もせず、ユーザーの問いや気持ちに応じてください。追加枠があれば考察を支える細部を添えてもよいですが、毎回補強する必要はありません。
...
既に語ったことを踏まえて答えます。新しい裏付けを添えるかは、今回の追加枠と会話に合わせて選んでください。
...
/** 追加枠と参考材料だけを渡す。疑い・考察への返答手順は指定しない。 */
export function formatDirective(directive: TurnDirective): string {
  const lines = [directive.maxNewLies === 0
    ? "新しい嘘の追加枠: 0。既に語ったこと・本物の設定・感想で応じてください。"
    : "新しい嘘の追加枠: 最大1つ。場面の細部を返答に自然に混ぜられる場合だけ使い、使わずに終えてもかまいません。"];
  if (directive.doubted) {
    lines.push("疑われている可能性のある設定:", directive.doubted.length
      ? directive.doubted.map((f) => `- ${f.claim}`).join("\n")
      : "（特定できていない。直前の会話を参照）");
  }
  if (directive.theory) lines.push("言及されている可能性のあるとしおの考察:", directive.theory);
  return lines.join("\n");
}
