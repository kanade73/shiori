...
- PR #28（`fix/issue-26`: issue #26 の上書き誤判定 + issue #11 の API キー2本の切り替え）と、issue #30・#32(a)・#33 の PR 3本が dev へのマージ待ち
  - `fix/issue-26` → PR #28（Closes #26, #11）。下の「API キー2本の自動切り替え」「嘘が本物の設定の『上書き』と誤判定される問題」の節
  - `fix/issue-30-toshio-trigger` → PR #34（Closes #30）
  - `fix/issue-32-no-topic-canon` → PR #35（Refs #32。(a) だけ）
  - `fix/issue-33-arc-aliases` → PR #36（Refs #33。一部）。**PR #28 の後にマージする**（ユーザー判断）。HANDOFF の衝突を避けるため `fix/issue-26` をこのブランチにマージ済みで、この HANDOFF は #28 と #30・#32(a)・#33 の両方の節を持つ。#28 より先にマージすると #28 の変更も一緒に入るので注意
- #30・#32(a)・#33 の3本は dev `e38df88` から切った（ユーザー指示「issue を簡単そうなものから3つ選んで解決して」→「PR まで作って」）。作業記録は下の「issue #30・#32(a)・#33」の節。この節は #36 にだけ入っている
