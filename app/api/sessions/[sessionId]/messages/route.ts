  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        const assistantMessage = await appendMessage(sessionId, "assistant", generation.message, "shiori", expression);
        ...
        const fallbackMessageRecord = await appendMessage(sessionId, "assistant", fallback, "shiori", FALLBACK_EXPRESSION);
