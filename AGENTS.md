...
`grounding=fabricated` の claim は正規化（別名→正式名）した上で `FabricatedFact` として `.data/db.json` に保存し、次の発話から材料に含める。これが「矛盾しない嘘」の実体。としおの発言は主張として記録しない（記録するキャラはシオリ1人）。**ユーザーがとしおの考察について聞いたら、関連する考察を参考材料として渡す**: `directive.ts` の `theoryInQuestion` が「としおの直後の発話（感想だけは除く）」か「としおの文の引用（bigram の重なり）」を拾う。これは言及の候補であり、補強の義務にはしない。シオリはとしおの考察を否定も断定的な肯定もせず、ユーザーの質問や感情に応じる。新しい嘘の追加枠があれば、支える細部を添えてもよい。履歴上のとしおの発言は直近1件だけ `generate.ts` が `【としお】` の印を付け 300 字に切ってシオリに渡す（それより古いものは落とす）。

**生成の役割分担**: `directive.ts` は新しい嘘の追加枠 `maxNewLies: 0 | 1` と参考材料（疑われている可能性のある設定・としおの考察）を渡す。感想、キャラ・出来事への言及のない雑談、直近2件連続で新しい嘘を保存した後は枠0。それ以外は最大1つを追加してよいが、追加は任意。疑いやとしおの材料があっても枠0を上書きしない。LLM がユーザーへの応じ方と枠を使うかを決める。上限はプロンプトで伝え、超過だけを理由に再生成・棄却はしない（整合性の検査と分離）。ケース別の返答手順をプロンプトに積み増さない。

**設計上の原則: 発想は縛らず、整合だけ縛る。** generate に候補選別やスコアリングを噛ませない。LLM が突飛なことを言うのが面白さの源で、構造化はあくまで事後の整合性チェックに限る。矛盾以外の理由で嘘を棄却しないこと。

矛盾判定のルールは `lib/server/claims.ts` にある。`identity / origin / lives_in / first_appeared` は1主語につき1値、`likes/dislikes` と `can/cannot` は対、同じ三つ組の肯定と否定は矛盾。それ以外は共存を許す。`evaluate.ts` の本物の設定への照合も `claims.contradictionReason` の同じ規則を使う（canon の自由記述 relation は閉じた語彙と一致するときだけ照合）。`did` の別の行動や `has` の別の持ち物を上書き扱いしない。明確な矛盾が1件あれば再生成し、他の主張で矛盾の割合が薄まっても通さない。テストは `npm test`（vitest。テストは対象の隣に `*.test.ts` として置く）。
