...
開発中は `LLM_PROVIDER=ollama` でローカルの Ollama（既定 `qwen3:8b`、`OLLAMA_MODEL` で変更）に差し替えられる（`lib/server/llm/provider.ts`）。キーが無く `LLM_PROVIDER` も未指定なら自動で ollama に落ちる。**ローカルはパイプラインの機構を無料で回すためのもので、嘘の面白さ・口調・claims の網羅性の評価には使わない**（8B クラスでは主語の取り違えや否認の弱さが出る。それはモデルの限界であって設計の評価ではない）。デプロイ先は常に anthropic。
...
- **LoRA / ローカルLLM での口調維持** — 口調はプロンプトのみで維持する方針。崩れることが確認できるまで入れない。勝手に学習パイプラインを組み始めないこと（開発用プロバイダとしての Ollama はこれとは別。上記参照）
