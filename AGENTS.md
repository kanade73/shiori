...
「草むしり検定はハコワレが頑張っていた」→「ハコワレ？ ハチワレのこと？」、続けて「ナックルとユピーの戦いは感動した」→「それ、HUNTER×HUNTER の話じゃない？」が成立すること。作品名・キャラ名はコードに書かず、`entities` と外部資料とモデルの一般知識で捌く。
...
- **別の作品**（`lib/server/other-work.ts`、2段）: ゲート（API を呼ばない）は、発話のカタカナの語のうち `entities` / `arcs` / 作品名のどれでもなく、会話の一般語（`names.ts` の `COMMON_KATAKANA`。アニメ・シーン・キャラなど作品を問わない語）でもなく、**外部資料の本文のカタカナの語にも無い**ものだけ残す（資料が本作の語彙を広く含むので、脇役・用語・擬音はここで落ちる。語ごとの一致で、部分一致にしない。「ナックル」が記事の「ナックルダスター」に吸われた）。残った語があれば判定役（`llm/other-work.ts`、`GEMINI_ROUTER_MODEL`・作品名と発話と語だけの小さな文脈）に「別の作品のものか、なら作品名」を聞く。web 検索はしない（無料枠では検索グラウンディングが 429）。モデルの一般知識で分からなければ空文字 = 本作の話として続ける。別の作品なら `directive` は `other_work` で、話題の特定・切り替わりの判定（資料の検索・埋め込み）は走らせず、シオリは短く指摘するだけ。相手の作品の中身も本作の設定も語らない
- どちらの回も extract を呼ばない（相手の作品の登場人物についての文が「本作の嘘」として保存されるため）。としおも割り込まない（`runToshioInterjection` に `directive` を渡す）
- ゲートで確認した挙動: 「ナックル・ユピー」「ゴン・キルア」→ HUNTER×HUNTER、「サトシ・ピカチュウ」→ ポケットモンスター、「ポシェットがボロボロ」→ 本作の話（`ボロボロ` は一般語で判定役まで行かない）。実 API の通しでルート1・2とも issue の想定どおりの返答を得た

### 会話パイプライン

`lib/server/llm/pipeline.ts`。1発話ごとに以下を回す。

-1. **names / other-work** — 登場人物の名前の誤字を正式名に直し（上の「名前の誤字と別の作品」）、見知らぬ固有名詞があれば別の作品の話かを判定する。別の作品なら topic は飛ばす
...
lib/
  server/
    ...
    names.ts                        発話の名前の扱い（登場人物の名前の誤字の近似一致・見知らぬカタカナ語。issue #1 ナックルベンチ）
    other-work.ts                   別の作品の話かの判定（ゲート: 外部資料の語彙 → 判定役）
    rate-limit.ts
    sse.ts                          Route Handler が text/event-stream を書くための口
    types.ts                        データモデル（答え合わせ専用の型は reveal/types.ts）
    llm/                            router（切り替わりの判定役）/ other-work（別の作品の判定役）/ topic（資料係）→ analyze → directive → generate → extract → evaluate → pipeline（+ toshio: としおの割り込み）
...
GEMINI_ROUTER_MODEL=     # 省略可。話題の切り替わりの判定役と、別の作品の判定役。既定 gemini-3.1-flash-lite
